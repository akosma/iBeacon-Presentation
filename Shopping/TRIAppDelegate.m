...
- (BOOL)application:(UIApplication *)application
    ...
    if (self.lastNotification == nil) {
        ...
        [defaults setObject:past forKey:LAST_NOFIFICATION_SENT];
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    if (status == kCLAuthorizationStatusAuthorizedAlways) {
        ...
        self.region =
            [[CLBeaconRegion alloc] initWithProximityUUID:uuid identifier:@"com.trifork.Shopping"];
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    if (state == CLRegionStateInside) {
        ...
        if (interval > 600 && app.applicationState == UIApplicationStateBackground) {
            ...
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:now forKey:LAST_NOFIFICATION_SENT];
            [defaults synchronize];
        }
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    NSPredicate *predicateRelevantBeacons =
        [NSPredicate predicateWithFormat:pred1, CLProximityNear, CLProximityImmediate];
    ...
    NSPredicate *predicateClosest =
        [NSPredicate predicateWithFormat:pred2, relevantsBeacons, relevantsBeacons];
